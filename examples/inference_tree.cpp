...
void image_inference(std::string image_file, std::string unary_file, std::string tree_file, std::string dataset_name, 
            ...
            float bil_spcstd, float bil_colstd, float bil_potts)
{
    ...
    std::cout << "Number of meta-labels = " << M << std::endl;

    MatrixXf unary_full = Eigen::MatrixXf::Zero(M, unaries.cols());
    unary_full.block(0, 0, L, unaries.cols()) = unaries;
    DenseCRF2D crf(size.width, size.height, M);
    crf.setUnaryEnergy(unary_full);
    ...
    found = tree_file.find_last_of("/\\");
    std::string tree_name = tree_file.substr(found+1);
    found = tree_name.find_last_of(".");
    tree_name = tree_name.substr(0, found);
    
    image_name = image_name + "_" + tree_name + "_" + method + "_" + std::to_string(spc_std) + "_" + std::to_string(spc_potts) + "_" + std::to_string(bil_spcstd) + "_" + std::to_string(bil_colstd) + "_" + std::to_string(bil_potts);
